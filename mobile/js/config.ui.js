...
iConfigure.prototype.ui.prototype = {
	...
	checkb: function( ctrl, no ){
		...
		var checked  = this._isChecked(ctrl.value) ? 'checked="checked"' : '',
			star 	 = ctrl.required ? ' *' : '',
			self     = this,
			csticClass    = this.getCsticClass(ctrl);
		var disbld = this.selectable ? '' : 'style="color:#B6B6B6;"';
	
		var $control = $('<label class="'+csticClass+'" for="'+ctrl.label+'">'+ctrl.label+''+star+'</label>');
	...
	combob: function( ctrl, no ){
		...
		var self = this,
			...
			csticClass = self.getCsticClass(ctrl),
			enabledValues   = [],
			dissabledValues = [];
        ...
        var $control = $('<div><label class="'+csticClass+'" for="'+ctrl.label+'">'+ctrl.label+''+star+'</label></div>');
	...
	radiob: function( ctrl, no ){
		...
		var length       = ctrl.Value.length, layoutType,
			star 		 = ctrl.required ? ' *' : '',
			csticClass   = this.getCsticClass(ctrl),
			isEmpty      = this.hasEmpty(ctrl.Value),
    	...
    	if (layoutType !== '') {
    		layoutWidth = "style='width:" + ( 400 - 50 ) / length + "px;'";
		}
